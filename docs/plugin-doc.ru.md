...
У команды должна быть одна основная метрика: это та метрика, изменение которой для нас важнее всего.

Кроме того у команды должны быть второстепенные метрики, которые не позволяют системе достигать изменения основной метрики любой ценой. 
То есть это метрики, которые, по возможности, не должны ухудшаться при улучшении основной метрики.

Процессы чаще всего балансируют на грани некоторого экстремума 
(например, максимальной пропускной способности при минимальном времени поставки),
поэтому улучшение одной метрики часто приводит к ухудшению другой.
Именно поэтому нам надо выбрать какую-то одну основную метрику, чтобы улучшать её, даже ценой незначительного ухудшения второстепенных метрик.
Если выбрать несколько главных метрик, то мы будем метаться между ними, не добиваясь улучшения ни одной из них.

На этом экране две метрики: проход и запасы.
* Проход - это объём выполненной работы за период.
* Запасы - это накопленный объём незавершенной работы.
  Это насколько взятые нами обязательства суммарно застряли в процессе выполнения.
  Единица измерения: единиц прохода умноженное на число дней, когда эти единицы прохода были в работе.
  Показаны в виде японских свечей, которые показывают объём незавершенной работы в начале и в конце периода,
  а так же максимум и минимум объёма незавершенной работы за период.

Почему вместо временн**ы**х метрик (t2m, lead time, cycle time) мы смотрим на накопленный объем незавершенной работы?
Дело в том, что временные метрики появляются только после завершения задач. 
Что очень сильно замедляет обратную связь.
А накопленный объем незавершенной работы реагирует быстрее, как только наше изменение начинает влиять на процесс.
И при этом накопленный объем незавершенной работы включает в себя временные метрики. 

### Индикаторы

На этом экране собраны индикаторы, которые позволяют оценить, насколько хорошо работает система в целом 
и какие изменения в ней происходят.

* Процентили времён: обзорные данные по 50, 75 и 85 процентилям времён: 
  * Времени жизни задачи - от момента создания до момента завершения
  * Времени поставки - от момента принятия обязательства до момента завершения
  * Времени цикла - от момента начала работы над задачей до момента завершения
  * Времени повторной работы - потери времени на возвратах и переделках
  * Времени блокировок - потери времени на блокировках
* Накопленный объём незавершенки: степень протухлости наших обязательств 
  (мы взяли обязательство сделать, но насколько много и каков суммарный объём этих обязательств?):
  * AWIP Общий - это сумма времени в днях, проведенного всеми незавершенными задачами на доске.
  * AWIP в работе - это сумма времени в днях, проведенного всеми незавершенными задачами на доске в колонках с работой.
  * AWIP в ожидании - это сумма времени в днях, проведенного всеми незавершенными задачами на доске в колонках с ожиданием.
    Снижая WIP лимит вы в пмервую очередь воздействуете на этот параметр, 
    так как задачи перестают тратить время дожидаясь когда ими займутся.
    Если этот параметр больше, чем "В работе", то у вас огромные проблемы с впихиванием невпихуемого.
  * WIP - это средний объём незавершенной работы за период.
  * WIP / проп. сп. - это время, которое потребуется для реализации среднего объёма незавершенной работы при средней пропускной способности.
* Пропускная способность: это объём выполненной работы за период.
  * В поставке - это объём выполненной работы за период, измеренный временем поставки (Lead time).
  * В цикле - это объём выполненной работы за период, измеренный временем цикла (Cycle time).
